(function () {
  ...
  function currentSop() { return pack.sops.find((s) => s.id === sopId); }
  ...
  function fillSops() {
    const depts = [];
    sops.forEach((s) => { if (!depts.includes(s.department || "Other")) depts.push(s.department || "Other"); });
    $("dept").innerHTML = ["All departments"].concat(depts).map((d) => `<option value="${d}">${d}</option>`).join("");
  ...
  function renderSopOptions(dept) {
    ...
    $("sop").innerHTML = list.map((s) => `<option value="${s.id}">${s.id} · ${s.label}</option>`).join("");
  ...
  function kpis(bundle, result) {
    ...
    $("kpis").innerHTML = [
      ["TSR", tsr],
      ["Cases", bundle ? String(bundle.n) : "—"],
      ["Decision", result ? result.decision : "—"],
      ["HITL", result && result.interrupt ? "YES" : (result ? "no" : "—")],
      ["Unowned writes", bundle ? String(bundle.unowned_writes) : "—"],
  ...
  function showResult(result, bundle) {
    ...
    $("subtitle").textContent = `${sop.id} · ${sop.department || ""} / ${sop.function || sop.owner || ""} · ${pack.sops.length} SOPs · ${pack.cases.length} cases · ScopeGate below the model`;
    $("tools").innerHTML = (result.tool_calls || []).map((t) => {
      const gate = t.ok ? "allow" : t.reason;
      const klass = t.ok ? "ok" : "warn";
      return `<tr><td>${t.step}</td><td>${t.tool_name}</td><td>${t.tier || ""}</td><td class="${klass}">${gate}</td><td>${JSON.stringify(t.result)}</td></tr>`;
    ...
    $("expect").textContent = JSON.stringify({ expect: result.expect, actual: { decision: result.decision, exception_code: result.exception_code, interrupt: result.interrupt, fired_rule: result.fired_rule, correct: result.correct }, evidence: result.evidence }, null, 2);
  ...
  function showCase(id) {
    ...
    $("path").innerHTML = ""; $("tools").innerHTML = ""; $("writes").innerHTML = "";
    $("expect").textContent = JSON.stringify(raw.expect, null, 2);
    $("objects").textContent = JSON.stringify(raw.objects, null, 2);
  }
  $("dept").onchange = () => { renderSopOptions($("dept").value); lastBundle = null; selected = null; renderCases(); if (selected) showCase(selected); };
  $("sop").onchange = () => { sopId = $("sop").value; lastBundle = null; selected = null; renderCases(); if (selected) showCase(selected); };
  $("runOne").onclick = () => {
    const result = engine.runCase(pack, selected, sopId);
    if (!lastBundle || lastBundle.sop === "PORTFOLIO") lastBundle = { sop: sopId, n: 0, tsr: 0, interrupts: 0, violations: 0, unowned_writes: 0, results: [] };
    ...
    renderCases(); showResult(result, lastBundle);
  };
  $("runAll").onclick = () => { lastBundle = engine.runAll(pack, sopId); selected = lastBundle.results[0] && lastBundle.results[0].case_id; renderCases(); if (selected) showResult(lastBundle.results[0], lastBundle); };
  $("runPortfolio").onclick = () => { lastBundle = engine.runPortfolio(pack); selected = lastBundle.results[0] && lastBundle.results[0].case_id; renderCases(); if (selected) showResult(lastBundle.results[0], lastBundle); };
  fillSops(); renderCases(); if (selected) showCase(selected);
})();
